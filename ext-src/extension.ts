...
class KubernetesCommanderViewProvider implements vscode.WebviewViewProvider {
    ...
    if (resourceType) {
      ...
      if (describeResourceTypeResult) {
        if (describeResourceTypeResult.code === 0) {
          ...
          // // Open the document
          // let describeOutputDocument: vscode.TextDocument = await vscode.workspace.openTextDocument({
          //   language: 'plaintext',
          //   content: `kubectl ${describeCommand}\n\n${describeOutput}`
          // });
          // await vscode.window.showTextDocument(describeOutputDocument);
          this._view?.webview.postMessage({
            command: 'describeResourceType',
            resourceType,
            describeOutput
  ...
  async describeResource(resourceType: any, resource: any) {
    if (resourceType.name === 'helmreleases') {
      ...
      const helmGetAll = `get all ${resource.name} --namespace ${resource.namespace}`;
      const helmGetAllResult = await helm.api.invokeCommand(helmGetAll);
      if (helmGetAllResult && helmGetAllResult.code === 0) {
        vscode.workspace.openTextDocument({
          language: 'yaml',
          content: `# helm ${helmGetAll}\n\n${helmGetAllResult.stdout}`
        }).then((doc) => {
          vscode.window.showTextDocument(doc);
        });
      }
